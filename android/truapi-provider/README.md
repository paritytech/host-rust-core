...
- `ChainProvider.withFileWarmStore(directory)` / `ChainProvider.withWarmStore(store)` - build a provider that resumes each chain from stored finalized state instead of warp syncing from the chain spec's checkpoint. The plain `ChainProvider()` has nowhere to keep that state, so `warmUp` and `persist` throw on it rather than doing nothing. The directory comes from the app because the crate cannot discover one the platform will not evict: pass `context.filesDir.absolutePath`, not `cacheDir`, which the OS reclaims under storage pressure.
- `NativeWarmStore` - implement it to keep blobs somewhere of your own. `load(genesisHash)` answers the stored string or `null`, `save(genesisHash, blob)` replaces it, and both are 32-byte hashes as `ByteArray`. A store that cannot answer must throw: returning no blob means "nothing stored yet" and lets the next snapshot overwrite good state. Both are suspending functions awaited on the caller's own thread, so the implementation must not require `Dispatchers.Main`.
- `warmUp(genesisHash)` - read a chain's stored state in before connecting to it. Call it first: smoldot consumes a blob only on a chain's first add, so a later call cannot take effect and answers `false`.
- `persist(genesisHash)` - snapshot a connected chain's finalized state into the store. Answers `false`, without storing, for a chain that has finalized nothing yet or that this provider has no open connection to. It is a full round trip through the light client, so drive it while the app is alive and treat a call from `onPause` as best effort.
- `ChainProviderException` - `Connect` when the genesis is outside the catalog or the transport fails, `BadGenesis` when the hash is not 32 bytes, `Listener` when the host's listener failed in a way it did not declare, `WarmStore` when the store failed or the provider has none.
