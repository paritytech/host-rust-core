...
Wire ids are append-only: existing ids never change, so deployed products stay compatible across protocol revisions. Discriminant 255 is permanently reserved for a correlated protocol error, allowing a host to reject API messages introduced after that host was released instead of leaving the caller pending.
