...
/** Provider surface {@link startWarmStartPersistence} drives. */
export interface PersistableProvider {
  persist(genesisHashHex: string): Promise<boolean>;
}

/** Options for {@link startWarmStartPersistence}. */
export interface PersistenceOptions {
  /** Delay before the first round. Defaults to 30 seconds. */
  initialDelayMs?: number;
  /** Gap between rounds after the first. Defaults to 60 seconds. */
  intervalMs?: number;
  /**
   * Called when a round fails for one chain. Defaults to `console.warn`,
   * because a store that silently stops writing looks exactly like one that is
   * working until the next cold start.
   */
  onError?: (error: unknown, genesisHashHex: string) => void;
}
...
/**
 * Snapshot each chain into the warm store on a timer, and keep doing it until
 * the returned function is called.
 *
 * A snapshot is only worth taking once the chain has finalized something, which
 * is why the first round waits: `persist()` resolves `false` until then, and
 * every round is a round trip through the light client rather than a write.
 *
 * Rounds never overlap. If one is still running when the next tick arrives, the
 * tick is skipped rather than queued, so a slow chain cannot build a backlog of
 * snapshots that all describe the same state.
 *
 * This is the web's substitute for saving on the way out. A provider usually
 * runs in a Web Worker, which sees neither `pagehide` nor `visibilitychange`,
 * and neither event keeps an async snapshot alive long enough to land.
 */
export function startWarmStartPersistence(
  provider: PersistableProvider,
  genesisHashes: readonly string[],
  ...
  const onError =
    options.onError ??
    ((error: unknown, genesisHashHex: string) => {
      console.warn(
        `[truapi-provider] could not store finalized state for ${genesisHashHex}`,
        error,
      );
    });

  const chains = [...genesisHashes];
  let stopped = false;
  let running = false;
  ...
  async function round(): Promise<void> {
    ...
    running = true;
    try {
      for (const genesisHashHex of chains) {
        if (stopped) return;
        try {
          await provider.persist(genesisHashHex);
        } catch (error) {
          onError(error, genesisHashHex);
        }
      }
    } finally {
      running = false;
  ...
  const initial = setTimeout(() => {
    void round();
    if (stopped) return;
    interval = setInterval(() => void round(), intervalMs);
  }, initialDelayMs);
