...
export interface BrowserNativeChatFileSourceStore {
  putSources(sources: readonly { sourceId: string; blob: Blob }[]): Promise<void>;
...
export function createBrowserNativeChatFilesHost(sourceStore?: BrowserNativeChatFileSourceStore): BrowserNativeChatFilesHost {
  let disposed = false;
  ...
  async function store<T>(mode: IDBTransactionMode, action: (store: IDBObjectStore) => IDBRequest<T>, cleanup = false): Promise<T> {
    ...
    return new Promise<T>((resolve, reject) => {
      ...
      const transaction = db.transaction(SOURCE_STORE, mode, { durability: "strict" });
      const request = action(transaction.objectStore(SOURCE_STORE));
      transaction.oncomplete = () => {
        if (disposed) { db.close(); database = undefined; }
  ...
  function prompt<T>(
    ...
    return new Promise<T | undefined>((resolve, reject) => {
      ...
      dialog.oncancel = (event) => { event.preventDefault(); if (!busy) abort(); };
      try {
        ...
        accept.onclick = () => {
          ...
          Promise.resolve(result).then((value) => {
            ...
            if (settled) return;
            busy = false;
            if (error instanceof DOMException && error.name === "AbortError") { abort(); return; }
            settled = true;
            close();
  ...
  async function abortExport(id: string, entry: ExportRecord): Promise<void> {
    exports.delete(id);
    try { await entry.writer.abort(); } catch { /* The writer may already be closed. */ }
    await entry.removePartial();
  }

  function withExport(id: string, action: (entry: ExportRecord) => Promise<void>): Promise<void> {
    const entry = exports.get(id);
    if (!entry) return Promise.reject(new Error("Chat file export is unavailable"));
    const operation = entry.queue.then(async () => {
      if (exports.get(id) !== entry) throw new Error("Chat file export is unavailable");
      await action(entry);
    });
  ...
  const host: BrowserNativeChatFilesHost = {
    async pickChatFiles(request) {
      ...
      if (request.maxFiles === 0) throw new Error("Chat file selection is unavailable");
      ...
      const files = await prompt("Send Chat attachments", request, "Attach files", (content) => {
        ...
        return () => {
          ...
          if (selected.length > request.maxFiles) throw new Error("Too many Chat attachments");
      ...
      if (sourceStore) {
        ...
        await new Promise<void>((resolve, reject) => {
          const transaction = db.transaction(SOURCE_STORE, "readwrite", { durability: "strict" });
          ...
          try {
            files.forEach((file, index) => {
              ...
              sources.add({ blob: file.slice(0, file.size, "application/octet-stream") } satisfies SourceRecord, picked[index]!.sourceId);
            });
          } catch {
            transaction.abort();
          }
      ...
      try {
        ...
        for (const file of picked) {
          ...
          const record = sourceStore
            ...
            : await store<SourceRecord | undefined>("readonly", (sources) => sources.get(file.sourceId));
          if (!record || !(record.blob instanceof Blob)) throw new Error("Chat file snapshot is unavailable");
    ...
    async readChatFile(sourceId, offset, length) {
      ...
      const record = sourceStore
        ...
        : await store<SourceRecord | undefined>("readonly", (sources) => sources.get(sourceId));
      if (!record || !(record.blob instanceof Blob)) throw new Error("Chat file source is unavailable");
      checkedSize(record.blob.size);
      if (offset + BigInt(length) > BigInt(record.blob.size)) throw new Error("Chat file read is out of bounds");
      const start = Number(offset);
      const bytes = new Uint8Array(await record.blob.slice(start, start + length).arrayBuffer());
      if (bytes.byteLength !== length) throw new Error("Chat file snapshot read failed");
      return bytes;
    },
    ...
    async beginChatFileExport(request) {
      ...
      const entry = await prompt("Save Chat attachment", request, "Choose destination", (content) => {
        ...
        return async () => {
          ...
          if (picker) {
            ...
            if (!navigator.storage?.getDirectory) throw new Error("Streaming Chat file export is unavailable");
            ...
            try { writer = await handle.createWritable(); } catch (error) { await removePartial(); throw error; }
          ...
          const result: ExportRecord = { writer, size: request.metadata.sizeBytes, written: 0, queue: Promise.resolve(), removePartial, presentCompleted };
          if (disposed) {
            try { await writer.abort(); } finally { await removePartial(); }
            return undefined;
          }
          return result;
        };
      ...
      if (disposed) { await abortExport(id, entry); return undefined; }
    ...
    async finishChatFileExport(exportId) {
      available();
      await withExport(exportId, async (entry) => {
        if (entry.written !== entry.size) throw new Error("Chat file export is incomplete");
    ...
    dispose() {
      ...
      for (const id of exports.keys()) void host.cancelChatFileExport(id).catch(() => {});
