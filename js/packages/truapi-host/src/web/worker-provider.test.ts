...
describe("createWebWorkerPairingHostRuntime", () => {
  ...
  it("keeps the worker alive until pending operations end", async () => {
    ...
    expect(indexOfKind(worker, "callbackResponse")).toBeLessThan(
      indexOfKind(worker, "dispose"),
    );
  ...
  it("routes string storage keys and drops updates after unsubscribe", async () => {
    const worker = new FakeWorker();
    ...
    await settle();
    expect(keys).toEqual(["chat/draft"]);
    expect(lastMessageOfKind(worker, "subscriptionItem")).toEqual({
      kind: "subscriptionItem",
      subId: 4,
      value: HostLocalStorageChangeItem.enc({ value: "0x07" }),
    });
    worker.emit({ kind: "subscriptionStop", subId: 4 });
    release.resolve();
    await settle();
