...
Worker runtime disposal waits for open product operations up to `operationGraceMs` (30 seconds by default).
The final `endOperation` reply reaches the worker before teardown. File callbacks remain available during that
grace period; teardown cancels unfinished exports and releases undelivered selections, but keeps delivered sources
for persisted native Chat actors.

Permission callbacks return `AllowOnce`, `AllowAlways`, or `Deny`, not booleans. `ChatAuthority` is a separate
review from `IdentityDisclosure`; identity consent does not grant Chat authority. `MainPurseChatPayment` uses
`confirmUserAction` to approve only the exact reviewed payment, never a reusable spending permission.
