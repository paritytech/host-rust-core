 ...
 *
 * `avgDurationMs` averages over ALL ops, not only completed ones: an op that is
 * still open contributes its elapsed span, so a stream full of hung calls does
 * not read as a fast session.
 */
