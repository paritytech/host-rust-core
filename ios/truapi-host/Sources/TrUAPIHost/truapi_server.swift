...
public protocol HostCallbacks: AnyObject, Sendable {
     ...
     * or always.
     */
    func devicePermission(product: NativeProductExecutionConfig, request: HostDevicePermissionRequest) async throws  -> NativePermissionDecision
     ...
     */
    func remotePermission(product: NativeProductExecutionConfig, request: RemotePermission) async throws  -> NativePermissionDecision
...
open func cancelNotification(id: UInt32)throws   {try rustCallWithError(FfiConverterTypeHostRejection_lift) {
     ...
     * or always.
     */
open func devicePermission(product: NativeProductExecutionConfig, request: HostDevicePermissionRequest)async throws  -> NativePermissionDecision  {
    return
        try  await uniffiRustCallAsync(
            rustFutureFunc: {
                uniffi_truapi_server_fn_method_hostcallbacks_device_permission(
                        self.uniffiCloneHandle(),FfiConverterTypeNativeProductExecutionConfig_lower(product),FfiConverterTypeHostDevicePermissionRequest_lower(request)
                )
            },
...
open func devicePermissionStatus(request: HostDevicePermissionRequest)async throws  -> NativeDevicePermissionStatus  {
     ...
     * Prompt the user for a remote permission `product` requested.
     */
open func remotePermission(product: NativeProductExecutionConfig, request: RemotePermission)async throws  -> NativePermissionDecision  {
    return
        try  await uniffiRustCallAsync(
            rustFutureFunc: {
                uniffi_truapi_server_fn_method_hostcallbacks_remote_permission(
                        self.uniffiCloneHandle(),FfiConverterTypeNativeProductExecutionConfig_lower(product),FfiConverterTypeRemotePermission_lower(request)
                )
            },
            pollFunc: ffi_truapi_server_rust_future_poll_rust_buffer,
            completeFunc: ffi_truapi_server_rust_future_complete_rust_buffer,
        ...
        devicePermission: { (
            ...
            let makeCall = {
                ...
                return try await uniffiObj.devicePermission(
                     product: try FfiConverterTypeNativeProductExecutionConfig_lift(product),
...
private let initializationResult: InitializationResult = {
    ...
    if (uniffi_truapi_server_checksum_method_hostcallbacks_device_permission() != 54714) {
        return InitializationResult.apiChecksumMismatch
    }
    if (uniffi_truapi_server_checksum_method_hostcallbacks_device_permission_status() != 21303) {
        return InitializationResult.apiChecksumMismatch
    }
    if (uniffi_truapi_server_checksum_method_hostcallbacks_remote_permission() != 60596) {
        return InitializationResult.apiChecksumMismatch
    }
    ...
    if (uniffi_truapi_server_checksum_method_nativetruapihostruntime_handle_sso_request() != 31041) {
        return InitializationResult.apiChecksumMismatch
    }
