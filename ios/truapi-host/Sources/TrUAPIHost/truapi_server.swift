...
open func notifyLocaleChanged(locale: HostLocaleSubscribeItem)  {try! rustCall() {
        uniffiCallStatus in
    uniffi_truapi_server_fn_method_nativetruapicore_notify_locale_changed(
            self.uniffiCloneHandle(),
        FfiConverterTypeHostLocaleSubscribeItem_lower(locale),uniffiCallStatus
    )
}
...
private let initializationResult: InitializationResult = {
    ...
    if (uniffi_truapi_server_checksum_method_nativetruapicore_notify_locale_changed() != 9965) {
        return InitializationResult.apiChecksumMismatch
    }
