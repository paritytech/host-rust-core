...
pub(super) struct Receiver {
    ...
    abort: AbortHandle,
    wake: mpsc::Sender<()>,
    permission_platform: Arc<dyn truapi_platform::Platform>,
    permission_scope: Option<u64>,
}
...
impl NativeChatRegistry {
    ...
    pub(super) async fn rebind_receiving(&self, context: &NativeChatContext, forgotten: &str) {
        ...
        let keys: Vec<_> = self
            ...
            .lock()
            .iter()
            .filter(|((owner, product), _)| *owner == wallet && product != forgotten)
            .map(|(key, receiver)| {
                (
                    key.clone(),
                    receiver.permission_platform.clone(),
                    receiver.permission_scope,
                )
            })
            .collect();
        ...
        let actors: Vec<_> = {
            ...
            keys.into_iter()
                .filter_map(|(key, platform, scope)| {
                    chats
                        .get(&key)
                        .cloned()
                        .map(|actor| (key.1, actor, platform, scope))
                })
                .collect()
        };
        for (product, actor, platform, scope) in actors {
            let mut context = context.background();
            context.permission_platform = platform;
            context.permission_scope = scope;
    ...
    pub(super) async fn ensure_receiving(
        ...
        if authorization.is_err()
            && receivers.get(&key).is_some_and(|receiver| {
                receiver.permission_scope != context.permission_scope
                    || !Arc::ptr_eq(&receiver.permission_platform, &context.permission_platform)
            })
        {
            return;
        }
        ...
        receivers.insert(
            ...
            Receiver {
                ...
                abort,
                wake,
                permission_platform: context.permission_platform.clone(),
...
pub(super) async fn require_authorized(
    ...
    let platform = context.permission_platform.as_ref();
    ...
    for request in [
        ...
        let once = context.foreground.as_ref().is_some_and(|grant| {
            grant.product == product
                && match request {
                    PermissionAuthorizationRequest::ChatAuthority => true,
                    PermissionAuthorizationRequest::Remote(_) => grant.statement_submit,
                    _ => false,
                }
        });
        if status == PermissionAuthorizationStatus::Denied
            || (status != PermissionAuthorizationStatus::Authorized && !once)
        {
...
pub(super) async fn require_upload_authorized(
    ...
    let once = context
        .foreground
        .as_ref()
        .is_some_and(|grant| grant.product == product && grant.preimage_submit);
    if status == PermissionAuthorizationStatus::Denied
        || (status != PermissionAuthorizationStatus::Authorized && !once)
    {
        return Err(ChatError::AccessNotGranted);
