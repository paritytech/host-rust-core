...
impl SigningHostRuntime {
    ...
    pub async fn pocket_purses(&self) -> Result<Vec<String>, v01::GenericError> {
        self.signing_host
            .pocket_purses()
            .await
    ...
    pub async fn pocket_list(
        &self,
        product_id: &str,
    ) -> Result<Vec<truapi::latest::ScarcityItem>, v01::GenericError> {
        self.signing_host
    ...
    /// A fresh receive key in `product_id`'s NFT purse for the wallet's own use,
    /// stable for a repeated `idempotency_key`.
    ...
    /// reporting progress; returns the including block once verified.
    pub async fn pocket_move_to_product(
        ...
        self.signing_host
            ...
            .await
            .map_err(|err| v01::GenericError {
                reason: err.to_service_error_reason(),
            })
    }
