...
use truapi::v01::{
    self, AccountId, AllocatableResource, DerivationIndex, GenesisHash, HostAccountSignVrfRequest,
...
use truapi_platform::{
    AccountAccessReview, AccountAliasReview, AuthPresenter, ChainProvider, CoreStorage,
    CoreStorageKey, CreateProofReview, CreateTransactionReview, Features, HostInfo,
    IdentityDisclosureReview, JsonRpcConnection, Navigation, Notifications,
    PermissionAuthorizationRequest, PermissionAuthorizationStatus, Permissions, PlatformInfo,
    PreimageHost, PreimageSubmitReview, ProductContext, ProductStorage, ResourceAllocationReview,
    RuntimeConfigValidationError, SignPayloadReview, SignRawReview, SignVrfReview,
    SigningHostConfig, StatementStoreProductSignReview, ThemeHost, UserConfirmation,
...
// FFI declarations for the user-confirmation review tree consumed by
// [`HostCallbacks::confirm_user_action`]. Every type reachable from
// [`UserConfirmationReview`] is declared `#[uniffi::remote(...)]`, so hosts
// receive the canonical structs and enums rather than mirrored copies. Each
// declaration restates the canonical definition field by field and the
