...
pub trait System: Send + Sync {
    ...
    /// [`RemotePermission::Remote`](crate::latest::RemotePermission::Remote)
    /// grant for the target host, and prompts for one on first use. dotNS names,
    /// `localhost`, and the app-handoff schemes (`mailto:`, `tel:`, `polkadot:`,
    /// `dot:`) consume no grant. The grant is per host and shared with outbound
    /// data access to it, so approving one covers the other.
    ///
    /// ```ts
