...
pub enum RemotePermission {
    ...
    /// caller identity the host derives and attaches (RFC 0025).
    ///
    ...
    Credential {
        ...
        /// the signature would otherwise travel in plaintext.
        domain: String,
...
mod tests {
    ...
    /// This enum is SCALE-encoded into the persisted permission key, so the
    /// variant order is the wire contract. Pin every discriminant: a reorder
    /// silently invalidates decisions users already made.
    #[test]
