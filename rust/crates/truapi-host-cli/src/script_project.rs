...
pub fn create(root: &Path, destination: Option<&Path>) -> Result<PathBuf> {
    ...
    let sdk = std::env::var("TRUAPI_SCRIPT_SDK").ok();
    create_with_sdk(root, destination, sdk.as_deref(), &types)
}
...
fn create_with_sdk(
    ...
    if let Some(sdk) = sdk {
        let sdk = if Path::new(sdk).exists() {
            format!("file:{}", fs::canonicalize(sdk)?.display())
        } else {
            sdk.to_string()
        };
        manifest["dependencies"]["@parity/product-sdk"] = sdk.into();
    }
...
mod tests {
    ...
    fn new_projects_carry_the_matching_declarations_without_reading_a_checkout() -> Result<()> {
        ...
        let script = create_with_sdk(root.path(), None, None, b"installed runner types")?;
        assert_eq!(
    ...
    fn creating_a_project_never_overwrites_an_existing_directory() -> Result<()> {
        ...
        assert!(create_with_sdk(root.path(), Some(&destination), None, b"types").is_err());
        assert_eq!(
    ...
    fn project_manifest_pins_the_sdk_and_can_resolve_a_renamed_entrypoint() -> Result<()> {
        let root = tempfile::tempdir()?;
        let script = create_with_sdk(root.path(), None, Some("0.30.0"), b"types")?;
