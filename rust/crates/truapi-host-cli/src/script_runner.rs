...
pub fn ensure_scratch_script_types(script: &Path) -> Result<()> {
    ensure_scratch_script_types_for_runner(script, &runner_path())
}

fn ensure_scratch_script_types_for_runner(script: &Path, runner: &Path) -> Result<()> {
    ...
    let types_path = script.with_extension("d.ts");
    fs::write(&types_path, runner_types)
    ...
    let contents = fs::read_to_string(script)
        .with_context(|| format!("read scratch script {}", script.display()))?;
    let reference = format!(r#"/// <reference path="./{types_name}" />"#);
    let has_reference = contents.lines().any(|line| line == reference);
    let has_module_marker = contents.lines().any(|line| line == "export {};");
    if has_reference && has_module_marker {
        return Ok(());
    }
    ...
    let updated = if has_reference {
        let reference_end = contents
            .find(&reference)
            .map(|start| start + reference.len())
            .context("scratch script type reference disappeared while migrating")?;
        let remainder = contents[reference_end..]
            .strip_prefix(line_ending)
            .unwrap_or(&contents[reference_end..]);
        format!(
            "{}{line_ending}export {{}};{line_ending}{remainder}",
            &contents[..reference_end]
        )
    } else {
        let header = if has_module_marker {
            reference
        } else {
            format!("{reference}{line_ending}export {{}};")
        };
        if contents.starts_with("#!") {
            let shebang_end = contents
                .find('\n')
                .map(|index| index + 1)
                .unwrap_or(contents.len());
            let remainder = contents[shebang_end..]
                .strip_prefix(line_ending)
                .unwrap_or(&contents[shebang_end..]);
            format!(
                "{}{line_ending}{header}{line_ending}{line_ending}{remainder}",
                &contents[..shebang_end]
            )
        } else {
            format!("{header}{line_ending}{line_ending}{contents}")
        }
    };
    let permissions = fs::metadata(script)
        .with_context(|| format!("read scratch script metadata {}", script.display()))?
        .permissions();
    let parent = script
        .parent()
        .with_context(|| format!("scratch script has no parent: {}", script.display()))?;
    let mut temporary = tempfile::NamedTempFile::new_in(parent)
        .with_context(|| format!("create temporary scratch script in {}", parent.display()))?;
    temporary
        .write_all(updated.as_bytes())
        .with_context(|| format!("write migrated scratch script {}", script.display()))?;
    fs::set_permissions(temporary.path(), permissions)
        .with_context(|| format!("preserve scratch script permissions {}", script.display()))?;
    temporary
        .persist(script)
        .map_err(|error| error.error)
        .with_context(|| format!("replace migrated scratch script {}", script.display()))?;
    Ok(())
}
...
"#
    ...
    #[test]
    fn legacy_scratch_scripts_gain_current_types_without_losing_source() -> Result<()> {
        let install = tempfile::tempdir()?;
        let runner = install.path().join(PACKAGED_RUNNER);
        fs::write(&runner, "packaged runner")?;
        fs::write(
        ...
        let scripts = tempfile::tempdir()?;
        let script = scripts.path().join("legacy.ts");
        fs::write(
            &script,
            "#!/usr/bin/env bun\n\nconst value = await truapi.system.getProductContext();\n",
        )?;

        ensure_scratch_script_types_for_runner(&script, &runner)?;
        ensure_scratch_script_types_for_runner(&script, &runner)?;
    ...
    #[test]
    fn referenced_scratch_scripts_gain_the_module_marker_once() -> Result<()> {
        let install = tempfile::tempdir()?;
        let runner = install.path().join(PACKAGED_RUNNER);
        fs::write(&runner, "packaged runner")?;
        fs::write(
        ...
        let scripts = tempfile::tempdir()?;
        let script = scripts.path().join("referenced.ts");
        fs::write(
            &script,
