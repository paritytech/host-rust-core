...
Previewnet's identity backend authenticates both username availability and
registration. The CLI obtains the short-lived Bearer token itself by proving
control of the signing account's derived `uid.dot` key, so auto-provisioning and
explicit mnemonics use the same identity. `paseo-next-v2` continues to use its
legacy unauthenticated backend. Both accept `--frame-listen <address>`
