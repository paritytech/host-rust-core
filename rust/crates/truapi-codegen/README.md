...
- Typed host callbacks, their WASM adapter and Worker proxy, and the Rust WASM bridge when `--platform-input` and the corresponding output options are supplied.
...
The platform bridges derive callback payloads and consent lifetimes from `truapi-platform`, including `PermissionDecision`, Chat authority and exact main-purse payment reviews, native Chat files, HOP transport, product operations, and storage subscriptions. Request dispatch carries the core's cancellation token into `CallContext`. Wire method addresses come from the canonical trait metadata, not fixture catalogs.
...
Run after any trait or type change in [`truapi`](../truapi/) or [`truapi-platform`](../truapi-platform/). If you only change runtime behavior without changing the protocol or callback shape, regeneration is not needed.
