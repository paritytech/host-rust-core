    ...
    P: Chain + Send + Sync + 'static,
{
    ...
    {
        let host = host;
        dispatcher.on_request(wire_table::CHAIN_RESOLVE_CHAIN, move |request_id: String, bytes: Vec<u8>| {
            let host = host.clone();
            Box::pin(async move {
                let request: versioned::chain::RemoteChainResolveChainRequest = match Decode::decode(&mut &bytes[..]) {
                    Ok(request) => request,
                    Err(err) => {
                        let error: truapi::CallError<versioned::chain::RemoteChainResolveChainError> =
                            truapi::CallError::MalformedFrame { reason: err.to_string() };
                        return Ok(encode_versioned_err_payload(
                            error,
                ...
                let target_version = request.version();
                let cx = CallContext::with_request_id(request_id.clone());
                let response: versioned::chain::RemoteChainResolveChainResponse = match host.resolve_chain(&cx, request).await {
                    Ok(value) => value,
                    Err(err) => {
                        return Ok(encode_versioned_err_payload(err, target_version));
                    }
